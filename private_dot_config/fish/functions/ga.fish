function ga
	set -l is_tracked_cmd 'git ls-files --error-unmatch {-1} >&2 2>/dev/null'
	set -l git_diff_cmd 'git diff --no-ext-diff --color=always -- {-1} | sed 1,4d'
	set -l cat_cmd 'bat --color=always --plain {-1}'

	set -l git_args -c color.status=always
	set -l fzf_args --preview="$is_tracked_cmd && $git_diff_cmd || $cat_cmd"
